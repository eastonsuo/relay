...
![Relay v0.2.0 软件界面](docs/relay-v0.2.0.jpg)
...
Relay `v0.2.0` 保持克制，只实现上下文切换所需的核心能力：
...
- 每个目标只记录三个条目：
  - **标题**：这个目标是什么；
  - **内容**：我要做什么；
  - **地址**：我在哪里做，可以多选软件，也可以补充目录或网址；
- 从 Relay 启动后开始统计前台软件切换频率；
- 常用软件优先展示，也可以从全部已安装软件中选择；
- 已选择的软件以纯文本按钮显示，点击即可启动或激活；
...
2. 下载 `Relay-v0.2.0.dmg`；
3. 打开 DMG，把 `Relay.app` 拖入 `Applications`；
4. 从“应用程序”中启动 Relay。

Release 同时提供 `Relay-v0.2.0.zip`。解压后把 `Relay.app` 拖入“应用程序”即可。
...
1. 点击 Tab 栏末尾的 `+` 创建目标；
2. 填写标题和内容；
3. 在“地址”中多选这个目标使用的软件，也可以补充目录或网址；
4. 点击软件名称，可以直接启动或切回对应软件；
5. 当前目标进入等待或阻塞时，选择对应状态；
6. 点击另一个 Tab，立即切换上下文；
7. 右键目标 Tab，可以归档或删除目标；
8. 点击归档箱，可以恢复已经归档的目标；
9. 点击右上角箭头收起窗口，只保留目标 Tab；
10. 需要编辑时再次展开。
