...
![Relay v0.1.1 软件界面](docs/relay-v0.1.1.jpg)
...
Relay `v0.1.1` 保持克制，只实现上下文切换所需的核心能力：
...
2. 下载 `Relay-v0.1.1.dmg`；
3. 打开 DMG，把 `Relay.app` 拖入 `Applications`；
4. 从“应用程序”中启动 Relay。

Release 同时提供 `Relay-v0.1.1.zip`。解压后把 `Relay.app` 拖入“应用程序”即可。
...
4. 点击另一个 Tab，立即切换上下文；
5. 右键目标 Tab，可以归档或删除目标；
6. 点击归档箱，可以恢复已经归档的目标；
7. 点击右上角箭头收起窗口，只保留目标 Tab；
8. 需要编辑时再次展开。
